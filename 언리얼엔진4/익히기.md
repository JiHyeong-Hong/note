...
Pawn 움직이기
1)  InputAxis + GetForwardVector -> AddMovementInput 또는 AddForce
2) RelativeLocation + MoveDistance + MakeVector -> MoveComponentTo

Actor 액터 움직이기
1) GetActorLocation + (GetWorldDeltaSeconds * GetForwardVector) -> SetActorLocation

주의할 점
1. 액터는 Input을 받을 수 없다.
2. 에디터에서 플레이 시 Pawn은 GameModeBase에서 DefaultPawnClass를 조작하고 싶은 오직 1개의 Pawn만 설정할 수 있다. 
3. 레벨에 2개 이상의 Pawn을 배치하면 조작할 수 없다. PlayerStart에 의해 자동 생성된 Pawn에서 조작을 시작한다.  
...
원하는 오브젝트 클릭 후 블루프린트 에디터에서 레퍼런스 바로 만들기  
